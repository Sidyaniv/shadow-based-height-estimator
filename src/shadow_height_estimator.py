...
from src.utils_for_estimate import find_heights_and_shadows
...
def rotate_shawdows_points(shawdows_points: Dict[int, List[Tuple[float]]],
                          center: Tuple[float],
                          rotate_angle: float) -> Tuple[float]:
    # Создаем матрицу поворота
    rotation_matrix = np.array([[np.cos(rotate_angle), -np.sin(rotate_angle)],
                                [np.sin(rotate_angle), np.cos(rotate_angle)]])
    dict_of_shawdows_points = {}
    for key in shawdows_points.keys():
        dict_of_shawdows_points[key] = []

        for point in shawdows_points[key]:
        # Вычисляем разность между координатами точки и координатами центра поворота
            dx = point[0] - center[0]
            dy = point[1] - center[1]
            ...
            dict_of_shawdows_points[key].append(coordinates)
    
    return dict_of_shawdows_points


def rotate_buildings_points(buildings_points: Dict[int, List[Tuple[float]]],
                          center: Tuple[float],
                          rotate_angle: float) -> Tuple[float]:
    # Создаем матрицу поворота
    rotation_matrix = np.array([[np.cos(rotate_angle), -np.sin(rotate_angle)],
                                [np.sin(rotate_angle), np.cos(rotate_angle)]])
    dict_of_buildings_points = {}
    for key in buildings_points.keys():
        dict_of_buildings_points[key] = []

        for point in buildings_points[key]:
        # Вычисляем разность между координатами точки и координатами центра поворота
            dx = point[0] - center[0]
            dy = point[1] - center[1]
            ...
            coordinates = (new_x, new_y)
            dict_of_buildings_points[key].append(coordinates)
    
    return dict_of_buildings_points
